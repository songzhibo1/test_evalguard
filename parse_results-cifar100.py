...
RESULTS_DIRS = [
    Path("results/surrogate_ft/cifar100"),
    Path("results/random_kw_fp/cifar100"),
    Path("results/distill/cifar100"),           
    Path("results/posthoc_prune/cifar100"),     
    Path("results/posthoc_quantize/cifar100")   
]
...
def extract_metrics(json_path, design_mode):
    ...
    if not raw_tag:
        if "__tag_" in filename:
            ...
            match_tag = re.search(r'(c100v5_[a-zA-Z0-9_]+)', filename)
    ...
    if exp_type == "random_kw_fp":
        ...
        tag_l = raw_tag.lower()
        if "c100v5_bgs_q010" in tag_l: exp_group = "ExpB1_BGS_Adaptive"
        elif "c100v5_bgs_qsweep" in tag_l: exp_group = "ExpB2_BGS_QSweep"
        elif "c100v5_bgs_tau" in tag_l: exp_group = "ExpB3_BGS_TauSweep"
        elif "c100v5_bgs_rw" in tag_l: exp_group = "ExpB4_BGS_RWSweep"
        elif "c100v5_fp_plain" in tag_l: exp_group = "ExpB5_Plain_Baseline"
        elif "c100v5_cross_" in tag_l: exp_group = "ExpA2_CrossArch"
        elif "c100v5_vtemp_" in tag_l: exp_group = "ExpA3_VTempSweep"
        elif "c100v5_seed_" in tag_l: exp_group = "ExpA4_MultiSeed"
        elif "c100v5_rw_" in tag_l: exp_group = "ExpA5_RWSweep"
        elif "c100v5_d_" in tag_l: exp_group = "ExpA6_DeltaSweep"
        elif "c100v5_beta_" in tag_l: exp_group = "ExpA7_BetaSweep"
        elif "c100v5_nq_" in tag_l: exp_group = "ExpA8_NQSweep"
        elif "c100v5_trig_" in tag_l: exp_group = "ExpA9_TrigSizeSweep"
        elif "c100v5_own_test_" in tag_l: exp_group = "ExpA13_OwnDataTest"
        elif "c100v5_own_" in tag_l: exp_group = "ExpA12_OwnDataMain"
        elif tag_l.startswith("c100v5_rw_0.0"): exp_group = "ExpA11_CleanBaseline"
        elif tag_l.startswith("c100v5_") and "_t" in tag_l: exp_group = "ExpA1_MainTable"
...
def main():
    ...
    if not json_files:
        print("⚠️ 未找到 JSON 结果文件！请确保 .sh 脚本已运行完毕并输出到了相应目录。")
        return
    ...
    output_csv = Path("results") / f"summary_results_c100_{args.design}_v5_full{file_suffix}.csv"
    output_df.to_csv(output_csv, index=False)
    print(f"✅ 解析完美完成！已修复 Tag 并提取所有隐藏参数 (Temp/NQ/Trig_Size)。CSV 保存至: {output_csv}")

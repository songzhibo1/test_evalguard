#!/bin/bash
# =============================================================================
# EvalGuard — CIFAR-100 Paper Experiments v5 — Part A (SOFT LABEL)
# =============================================================================
#
# This is File 1 of 3 (expA1 .. expA13).  exp numbering is continuous across:
#   Part A  run_cifar100_paper_v5_a_soft.sh     expA1 .. expA13
#   Part B  run_cifar100_paper_v5_b_hard.sh     expb1 .. expb5
#   Part C  run_cifar100_paper_v5_c_attacks.sh  expC1 .. expC7
#
# Built for the v6 watermark.py + new v5 additions:
#   - Target-aware margin, auto-scaled verify_temperature (vT = 15.0 for C=100)
#   - Adaptive τ quantile calibration       (Part B)
#   - SVHN cross-dataset query              (Part C)
#   - Post-hoc prune / INT8 quant attacks   (Part C)
#
# Available STEPS in THIS file:
#   expA1   Main table (soft):     3 pairs x 4 temps             12 jobs
#   expA2   Cross-arch β/δ combos: resnet56->resnet20             8 jobs
#   expA3   V_TEMP sensitivity sweep                              7 jobs
#   expA4   Multi-seed reproducibility                            5 jobs
#   expA5   Soft r_w sweep (accuracy × detection)                 5 jobs
...
#   expA12  own_trigger main table (3 pairs × T=5)                3 jobs
#   expA13  own_trigger + testset (zero-leakage verify)           1 job
#                                              Total = 61 jobs + 1 task

# Usage:
#   GPU_IDS="0,1,2,3" ./run_cifar100_paper_v5_a_soft.sh
#   GPU_IDS="0,1"     STEPS="expA1 expA12"   ./run_cifar100_paper_v5_a_soft.sh
#   GPU_IDS="0"       DRY_RUN=1              ./run_cifar100_paper_v5_a_soft.sh
# =============================================================================
...
# -----------------------------------------------------------------
# CIFAR-100 global hyper-parameters (v5, match the plan txt)
# -----------------------------------------------------------------
NQ=50000
DIST_EP=160
DIST_LR=0.002
...
JOB_FILE=$(mktemp /tmp/evalguard_c100v5a_jobs.XXXXXX)
JOB_COUNT=0
...
from evalguard.configs import (cifar100_data, cifar100_resnet20,
                                cifar100_vgg11, cifar100_resnet56, create_student)
...
# =============================================================================
# EXP A1 -- Main table (soft): 3 pairs x 4 temps = 12 jobs
# =============================================================================
...
# =============================================================================
# EXP A2 -- Cross-arch hardening (resnet56 -> resnet20), BETA/D combos = 8 jobs
# =============================================================================
if step_enabled expA2; then
    for COMBO in "0.7:15.0" "0.7:20.0"; do
        B_VAL="${COMBO%%:*}"; D_VAL="${COMBO##*:}"
        for T in 1 3 5 10; do
            TAG="c100v5_cross_b${B_VAL}_d${D_VAL}_T${T}"
            add_job "${TAG}" $(soft_args "cifar100_resnet56" "resnet20" "${T}" "${RW}" "${D_VAL}" "${B_VAL}" "${V_TEMP}" "${TAG}" "--seed 42")
        done
    done
fi
...
#   Fixed: resnet56->resnet56, T=5, D=15, BETA=0.5
# =============================================================================
if step_enabled expA3; then
    for VT_VAL in 1.0 3.0 5.0 10.0 15.0 20.0 30.0; do
        TAG="c100v5_vtemp_${VT_VAL}"
        add_job "${TAG}" $(soft_args "cifar100_resnet56" "resnet56" "${SWEEP_T}" "${RW}" "${D_LOGIT}" "${BETA}" "${VT_VAL}" "${TAG}" "--seed 42")
    done
fi
...
if step_enabled expA4; then
    for SEED in 42 123 256 777 2024; do
        TAG="c100v5_seed_${SEED}"
        add_job "${TAG}" $(soft_args "cifar100_resnet56" "resnet56" "${SWEEP_T}" "${RW}" "${D_LOGIT}" "${BETA}" "${V_TEMP}" "${TAG}" "--seed ${SEED}")
    done
fi


# =============================================================================
# EXP A5 -- Soft r_w sweep (captures accuracy × detection tradeoff) = 5 jobs
# =============================================================================
if step_enabled expA5; then
    for RW_VAL in 0.02 0.05 0.10 0.20 0.50; do
        TAG="c100v5_rw_${RW_VAL}"
        add_job "${TAG}" $(soft_args "cifar100_resnet56" "resnet56" "${SWEEP_T}" "${RW_VAL}" "${D_LOGIT}" "${BETA}" "${V_TEMP}" "${TAG}" "--seed 42")
    done
fi
...
if step_enabled expA6; then
    for D_VAL in 5.0 10.0 15.0 20.0 25.0; do
        TAG="c100v5_d_${D_VAL}"
        add_job "${TAG}" $(soft_args "cifar100_resnet56" "resnet56" "${SWEEP_T}" "${RW}" "${D_VAL}" "${BETA}" "${V_TEMP}" "${TAG}" "--seed 42")
    done
fi
...
if step_enabled expA7; then
    for B_VAL in 0.3 0.5 0.7 0.8 0.9; do
        TAG="c100v5_beta_${B_VAL}"
        add_job "${TAG}" $(soft_args "cifar100_resnet56" "resnet56" "${SWEEP_T}" "${RW}" "${D_LOGIT}" "${B_VAL}" "${V_TEMP}" "${TAG}" "--seed 42")
    done
fi
...
# =============================================================================
# EXP A10 -- Logit margin profiling (v6 target-aware, diagnostic inline)
#   Output: ${LOG_DIR}/margin_profile_target_aware_*.json
# =============================================================================
if step_enabled expA10; then
    echo ""
    echo "================================================================"
    echo " EXP A10 -- Logit margin profiling (target-aware, v6)"
    echo "================================================================"
    if [ "${DRY_RUN}" = "1" ]; then
        echo "  [DRY RUN] Would profile resnet56, vgg11, resnet20"
    else
...
import torch
sys.path.insert(0, os.path.abspath('.'))
from evalguard.configs import (cifar100_data, cifar100_resnet56,
                                cifar100_vgg11, cifar100_resnet20)
from evalguard.crypto import keygen, kdf
from evalguard.watermark import derive_target_class

LOG_DIR = sys.argv[1]
device = "cuda" if torch.cuda.is_available() else "cpu"
_, _, _, testloader = cifar100_data(batch_size=256)
NC = 100
K_w = kdf(keygen(256, seed=42), "watermark")

models = [("resnet56", cifar100_resnet56),
          ("vgg11",    cifar100_vgg11),
          ("resnet20", cifar100_resnet20)]
combos = [{"D":15.0,"BETA":0.5},{"D":15.0,"BETA":0.7},
          {"D":20.0,"BETA":0.5},{"D":20.0,"BETA":0.7}]
DELTA_MIN = 0.5

for name, fn in models:
    print("\n--- {} ---".format(name))
    m = fn(pretrained=True).to(device).eval()
    ...
    out = {"model":name,"dataset":"cifar100","n_samples":int(len(margins_target)),
...
# =============================================================================
# EXP A11 -- r_w = 0.0 clean baseline (from supplement) = 1 job
# =============================================================================
...
# =============================================================================
# EXP A12 -- own_trigger main table (from supplement) = 3 jobs
#   Owner reconstructs the trigger set from private data.
# =============================================================================
if step_enabled expA12; then
    for PAIR in "cifar100_resnet56:resnet56" "cifar100_resnet56:resnet20" "cifar100_vgg11:vgg11"; do
        TEACHER="${PAIR%%:*}"; STUDENT="${PAIR##*:}"
        SHORT_T="${TEACHER#cifar100_}"
        TAG="c100v5_own_${SHORT_T}_${STUDENT}"
        ARGS="--experiment surrogate_ft \
...
# =============================================================================
# EXP A13 -- own_trigger + testset (from supplement) = 1 job
#   True zero-leakage probe using testset.
# =============================================================================
if step_enabled expA13; then
    TAG="c100v5_own_test_resnet56_resnet56"
    ARGS="--experiment surrogate_ft \
--model cifar100_resnet56 --student_arch resnet56 \
--label_mode soft --trigger_mode own_trigger \
...
echo ""
echo "============================================================"
echo " EvalGuard -- CIFAR-100 v5 Part A (SOFT)  expA1..expA13"
...
WORKER_PIDS=()
FAIL_FILE=$(mktemp /tmp/evalguard_c100v5a_fails.XXXXXX)
...
echo ""
echo "============================================================"
echo " DONE -- CIFAR-100 v5 Part A  $(date '+%Y-%m-%d %H:%M:%S')"
echo " Logs: ${LOG_DIR}/"
if [ -s "${FAIL_FILE}" ]; then

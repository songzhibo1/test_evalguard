...
# 专门指向 Tiny-ImageNet 的结果目录
RESULTS_DIRS = [
    Path("results/surrogate_ft/tinyimagenet"),
    Path("results/random_kw_fp/tinyimagenet"),
    Path("results/distill/tinyimagenet"),           
    Path("results/posthoc_prune/tinyimagenet"),     
    Path("results/posthoc_quantize/tinyimagenet")   
]
...
def extract_metrics(json_path, design_mode):
    ...
    # 过滤掉不需要解析表格的实验（比如 fidelity 和 overhead，它们在 Exp5 和 Exp6 中单独打印）
    if exp_type not in ["surrogate_finetune", "random_kw_fp", "posthoc_prune", "posthoc_quantize", "distillation"]:
        return []

    watermark = data.get("watermark_config") or {}
    baseline = data.get("distillation_baseline") or data.get("baseline_result") or {}
    ...
    if exp_type == "distillation" and not is_svhn: 
        return []
    ...
    # --- 完美对齐 run_tinyimagenet_paper.sh (Exp1 ~ Exp9) 的分组逻辑 ---
    if exp_type == "random_kw_fp":
        exp_group = "Exp8_Hard_FP_Scan" if is_hard else "Exp4_Soft_FP_Scan"
    elif exp_type == "posthoc_prune":
        exp_group = "Exp8_Hard_Prune" if is_hard else "Exp3_Soft_Prune"
    elif exp_type == "posthoc_quantize":
        exp_group = "Exp3_Soft_Quantize"
    elif exp_type == "surrogate_finetune":
        if "clean_baseline" in tag_l:
            exp_group = "Exp9_Clean_Baseline"
        elif "own" in tag_l or trigger_mode == "OWN":
            exp_group = "Exp7_Zero_Leakage"
        elif "bgs" in tag_l or is_hard:
            exp_group = "Exp2_Hard_BGS"
        else:
            exp_group = "Exp1_Soft_Main"
    elif exp_type == "distillation" and is_svhn:
        exp_group = "SVHN_CrossDomain"
    else:
        exp_group = "Legacy_Or_Unknown"
    ...
    nq_val = watermark.get("nq", data.get("nq", dist_conf.get("n_queries", "N/A")))
    if nq_val == "N/A":
    ...
    record_common = {
        ...
        "NQ": nq_val,
        "Trig_Size": trig_size,
        "T_Acc": format_acc(teacher_info.get("accuracy_original", teacher_info.get("accuracy", "N/A"))),
        "Base_Acc": format_acc(student_acc),
    }

    records = []

    # 1. Random K_w FP Scan
    if exp_type == "random_kw_fp":
    ...
    # 4. Surrogate Fine-Tuning (主表数据 Exp1, Exp2, Exp7, Exp9)
    elif exp_type == "surrogate_finetune":
...
def main():
    ...
    # 默认提取 single_ctrl 用于对比
    parser.add_argument("--design", type=str, default="single_ctrl", choices=["mean_rest", "single_ctrl", "suspect_top1", "all"])
    ...
    if not json_files:
        print("⚠️ 未找到 JSON 结果文件！请确保 run_tinyimagenet_paper.sh 脚本已运行完毕。")
        return
    ...
    for exp_name, exp_group in df.groupby("Experiment", sort=False):
        ...
        for trig_name, trig_group in exp_group.groupby("Trigger", sort=False):
            ...
            for arch_pair, arch_group in trig_group.groupby("Arch_Pair", sort=False):
                ...
                if "RandKw" in exp_name or "FP_Scan" in exp_name:
    ...
    output_csv = output_dir / f"summary_results_tinyimg_{args.design}_v6{file_suffix}.csv"
    output_df.to_csv(output_csv, index=False)
    print(f"✅ Tiny-ImageNet 实验解析完成！CSV 已保存至: {output_csv}")

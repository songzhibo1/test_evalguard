#!/bin/bash
# =============================================================================
# EvalGuard — CIFAR-100 Paper Experiments v5 — Part C (FP + ATTACKS)
# =============================================================================
#
# This is File 3 of 3 (expC1 .. expC7).  exp numbering continues from Part B.
#   Part A  run_cifar100_paper_v5_a_soft.sh     exp1  .. exp10
#   Part B  run_cifar100_paper_v5_b_hard.sh     exp11 .. exp15
#   Part C  run_cifar100_paper_v5_c_attacks.sh  expC1 .. expC7   <-- this file
#
# Section C (verification integrity, random K_w' FP scan) and
# Section D (adversarial post-hoc attacks: SVHN cross-dataset, magnitude
# pruning, INT8 quantization).
#
# CACHE SHARING — for this part to be fast, run Part A (exp1) and Part B
# (exp11) FIRST.  expC1/expC4/expC5 reuse cached students from Part A, and
# expC2/expC6 reuse cached students from Part B.  The tags below match
# exactly so the cache hits.
#
...
#   expC7  Cross-dataset SVHN query attack on BGS hard-label  1 job
#                                                 Total =    13 jobs
#
# Usage:
#   GPU_IDS="0,1,2,3" ./run_cifar100_paper_v5_c_attacks.sh
#   GPU_IDS="0"       STEPS="expC3 expC7"  ./run_cifar100_paper_v5_c_attacks.sh
#   GPU_IDS="0"       DRY_RUN=1            ./run_cifar100_paper_v5_c_attacks.sh
# =============================================================================
...
HARD_TAU_Q=0.10
HARD_TAU_CALIB=3000  # <--- [新增]: 为硬标签自适应tau校准补充的全局参数
N_RAND_KW=100
...
JOB_FILE=$(mktemp /tmp/evalguard_c100v5c_jobs.XXXXXX)
JOB_COUNT=0
...
from evalguard.configs import (cifar100_data, svhn_data,
                                cifar100_vgg11, cifar100_resnet56, create_student)
_ = cifar100_data();                        print("  CIFAR-100 data ready.")
_ = svhn_data(normalize_for="cifar100");    print("  SVHN data ready.")
...
# =============================================================================
# expC1 -- Random K_w' FP scan (soft, N=100) = 3 jobs
#   Reuses exp1 cached students (tag pattern c100v5_<t>_<s>_T5).
# =============================================================================
...
#   Reuses exp11 cached students (tag c100v5_bgs_q010_<t>_<s>).
# =============================================================================
if step_enabled expC2; then
    for PAIR in "cifar100_resnet56:resnet56" "cifar100_vgg11:vgg11"; do
        TEACHER="${PAIR%%:*}"; STUDENT="${PAIR##*:}"
        SHORT_T="${TEACHER#cifar100_}"
        REUSE_TAG="c100v5_bgs_q010_${SHORT_T}_${STUDENT}"
...
# =============================================================================
# expC3 -- Cross-dataset SVHN query attack (soft) = 2 jobs
#   Attacker only sees 32x32 SVHN images; distill over watermarked soft labels.
# =============================================================================
if step_enabled expC3; then
    for PAIR in "cifar100_resnet56:resnet56" "cifar100_vgg11:vgg11"; do
        TEACHER="${PAIR%%:*}"; STUDENT="${PAIR##*:}"
        SHORT_T="${TEACHER#cifar100_}"
        TAG="c100v5_svhn_${SHORT_T}_${STUDENT}"
...
#   Reuses exp1 cache (tag c100v5_<t>_<s>_T5).
# =============================================================================
...
#   Reuses exp11 cache (tag c100v5_bgs_q010_resnet56_resnet56).
# =============================================================================
if step_enabled expC6; then
    REUSE_TAG="c100v5_bgs_q010_resnet56_resnet56"
    TAG="prune_hard_resnet56_resnet56"
    ARGS="--experiment pruning_attack \
...
# =============================================================================
# expC7 -- [新增] SVHN cross-dataset query attack on BGS HARD-LABEL surrogate = 1 job
#   Parallel to expC3 (SVHN soft) but with hard-label + adaptive τ.
#   Pair: resnet56 -> resnet56.
# =============================================================================
if step_enabled expC7; then
    TAG="c100v5_svhn_hard_resnet56_resnet56"
    ARGS="--experiment distill \
--model cifar100_resnet56 --student_arch resnet56 \
--label_mode hard --hard_label_mode bgs \
...
echo ""
echo "============================================================"
echo " EvalGuard -- CIFAR-100 v5 Part C (FP + ATTACKS)  expC1..expC7"
echo " GPUs       : ${GPU_LIST[*]} (${NUM_GPUS})"
echo " STEPS      : ${STEPS}"
echo " Total jobs : ${JOB_COUNT}"
echo " Jobs/GPU   : ~$(( (JOB_COUNT + NUM_GPUS - 1) / NUM_GPUS ))"
echo " LOG_DIR    : ${LOG_DIR}"
echo " Cache reuse: expC1/C4/C5 <- Part A  ;  expC2/C6 <- Part B"
echo "============================================================"
...
WORKER_PIDS=()
FAIL_FILE=$(mktemp /tmp/evalguard_c100v5c_fails.XXXXXX)
...
echo ""
echo "============================================================"
echo " DONE -- CIFAR-100 v5 Part C  $(date '+%Y-%m-%d %H:%M:%S')"
echo " Logs: ${LOG_DIR}/"
if [ -s "${FAIL_FILE}" ]; then
